...
module.exports.JOB_BODY = async (config, log) => {
    ...
    return await RecipeOrder.findAll({
        where: {
                            ...
                            SELECT
                                ...
                                SUM(
                                    CASE WHEN fee_asset.is_base=true
                                        THEN (eof.quantity * eof.price) + COALESCE(eof.fee, 0) -- add raw fee if it's in base asset
                                        ELSE (eof.quantity * eof.price) + COALESCE(eof.fee * price, 0) -- calculate cost with price
                                    END
                                ) as fills_cost
