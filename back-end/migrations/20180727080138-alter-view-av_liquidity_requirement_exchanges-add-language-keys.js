...
module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query('DROP VIEW av_liquidity_requirement_exchanges')
    .then(() => {
      return queryInterface.sequelize.query(`
        ...
        ( SELECT
            ...
            ) AS last_week) AS last_week_vol,
            lh.timestamp_to AS last_updated,
            (CASE WHEN lh.volume > lr.minimum_volume THEN 'liquidity_exchanges.status.meets_liquidity_requirements' ELSE 'liquidity_exchanges.status.lacking' END) AS passes
