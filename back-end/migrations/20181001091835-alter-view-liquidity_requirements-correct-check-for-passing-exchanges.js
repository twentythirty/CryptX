...
module.exports = {
    ...
    return queryInterface.sequelize.query('DROP VIEW av_instrument_liquidity_requirements').then(() => {
      return queryInterface.sequelize.query(`
        ...
        ( SELECT
          ...
          , ( SELECT count(*) FROM (
                SELECT AVG(ilh.volume) AS volume, ilh.exchange_id, ilh.instrument_id FROM instrument_liquidity_history AS ilh 
  ...
  down: (queryInterface, Sequelize) => {
    return queryInterface.sequlize.query('DROP VIEW av_instrument_liquidity_requirements').then(() => {
      return queryInterface.sequelize.query(`
        CREATE OR REPLACE VIEW av_instrument_liquidity_requirements (
          id,
          instrument_id,
        ...
        ( SELECT
          ...
          END ) AS exchange_count
          , ( SELECT count(*) FROM (
                SELECT SUM(ilh.volume) AS volume, ilh.exchange_id, ilh.instrument_id FROM instrument_liquidity_history AS ilh 
                WHERE ilh.timestamp_to >= CURRENT_DATE - INTERVAL '1 day' * lr.periodicity_in_days
                GROUP BY ilh.exchange_id, ilh.instrument_id
          ) AS lh
            WHERE
