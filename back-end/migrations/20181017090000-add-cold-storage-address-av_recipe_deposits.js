...
module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query('DROP VIEW IF EXISTS av_recipe_deposits').then(done => {
      ...
      (
        SELECT dep.id,
          ...
          ex.name AS exchange,
          ( CASE WHEN exa.address IS NULL THEN csa.address ELSE exa.address END) AS account,
          dep.amount,
        ...
        FROM recipe_run_deposit dep
          ...
          LEFT JOIN exchange_account exa ON dep.target_exchange_account_id = exa.id
          LEFT JOIN exchange ex ON exa.exchange_id = ex.id
          LEFT JOIN cold_storage_account csa ON csa.id=dep.cold_storage_account_id
