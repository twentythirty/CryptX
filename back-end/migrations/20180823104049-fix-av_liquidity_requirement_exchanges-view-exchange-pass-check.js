...
module.exports = {
  up: (queryInterface, Sequelize) => {
    ...
    ( WITH newest_prices AS
      (
          SELECT
            DISTINCT ON( imd.exchange_id,imd.instrument_id ) imd.exchange_id
          , imd.instrument_id
          , imd."timestamp"
          , imd.ask_price
      ...
      SELECT
        DISTINCT ON( lh.exchange_id,lh.instrument_id ) lh.id
      , lh.exchange_id
      , ex.name AS exchange
      , lh.instrument_id
      , i.symbol AS instrument
      , exm.external_instrument_id AS instrument_identifier
      , ( SELECT np.ask_price FROM newest_prices np WHERE( ( lh.exchange_id = np.exchange_id ) AND( lh.instrument_id = np.instrument_id ) ) ) AS current_price
      , CASE
          WHEN(lh.timestamp_to >= CURRENT_DATE - '1 day'::INTERVAL)
          THEN lh.volume
          ELSE null
        END AS last_day_vol
      , ( SELECT sum( last_week.volume ) AS sum FROM ( SELECT ilh.volume FROM instrument_liquidity_history AS ilh WHERE( lh.exchange_id = ilh.exchange_id AND lh.instrument_id = ilh.instrument_id AND ilh.timestamp_to >= current_date - '7 days'::INTERVAL ) ) last_week ) AS last_week_vol
      , lh.timestamp_to AS last_updated
      , CASE
          WHEN( 
              ( SELECT sum( last_week.volume ) AS sum FROM ( SELECT ilh.volume FROM instrument_liquidity_history AS ilh WHERE( lh.exchange_id = ilh.exchange_id AND lh.instrument_id = ilh.instrument_id AND ilh.timestamp_to >= current_date - INTERVAL '1 day' * lr.periodicity_in_days ) ) last_week ) >= lr.minimum_volume
          )
          THEN 'liquidity_exchanges.status.meets_liquidity_requirements'::TEXT
          ELSE 'liquidity_exchanges.status.lacking'::TEXT
      END AS passes
      FROM ( ( ( ( instrument_liquidity_history lh
      LEFT JOIN exchange ex
       ON ( ( lh.exchange_id = ex.id ) ) )
      JOIN instrument_exchange_mapping exm
       ON ( ( ( lh.exchange_id = exm.exchange_id )
      AND ( lh.instrument_id = exm.instrument_id ) ) ) )
      LEFT JOIN instrument i
       ON ( ( lh.instrument_id = i.id ) ) )
      LEFT JOIN instrument_liquidity_requirement lr
       ON ( ( lh.instrument_id = lr.instrument_id ) ) ) 
