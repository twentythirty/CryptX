...
module.exports = {
  up: (queryInterface, Sequelize) => {
    ...
    CREATE OR REPLACE VIEW av_cold_storage_transfers (
      ...
      JOIN recipe_order AS ro ON cst.recipe_run_order_id = ro.id
      JOIN exchange AS ex ON ro.target_exchange_id = ex.id
      JOIN exchange_account AS exa ON ro.target_exchange_id = exa.exchange_id AND cst.asset_id = exa.asset_id
    )
    `)
  },

  down: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query('DROP VIEW av_cold_storage_transfers');
  }
};