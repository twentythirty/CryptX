...
module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query('DROP VIEW av_recipe_deposits').then(done => {
      ...
      (SELECT 
          ...
          dep.amount,
          (CASE WHEN dep.status=151 THEN (SELECT SUM(rrd.investment_percentage) FROM recipe_run_detail AS rrd WHERE dep.recipe_run_id = rrd.recipe_run_id AND dep.asset_id = rrd.quote_asset_id AND exa.exchange_id = rrd.target_exchange_id) ELSE NULL END) AS investment_percentage,
  ...
  down: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query('DROP VIEW av_recipe_deposits').then(() => {
      return queryInterface.sequelize.query(`
